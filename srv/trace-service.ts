...
export default class TraceService extends cds.ApplicationService {
  ...
  init() {
    ...
    // -----------------------------------------------------------------------
    // AnchorColdChain — anchor cold-chain telemetry on-chain
    // -----------------------------------------------------------------------
    this.on('AnchorColdChain', async (req) => {
      const { batchId, telemetryHash, minTemp, maxTemp, inRange, walletAddress, walletVkh } =
        req.data as {
          batchId: string;
      ...
      const metadata = {
        674: {
          msg: [`TRACE:COLD_CHAIN:${inRange ? 'OK' : 'BREACH'}`],
          batch: batch.batchNumber,
          hash: telemetryHash,
      ...
      await INSERT.into(ProofEvents).entries({
        batch_ID: batchId,
        eventType: 'DOCUMENT_ANCHOR',
        payloadDigest: telemetryHash,
        schema: 'COLD_CHAIN_LOG',
        signerVkh: walletVkh,
        buildId: result.buildId,
        signingRequestId: signingReq.signingRequestId,
        status: 'PENDING'
